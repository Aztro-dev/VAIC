...
impl Plugin for MainMenuPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(OnEnter(UIState::MainMenu), spawn_ui)
            .add_systems(OnExit(UIState::MainMenu), despawn_ui)
            .add_systems(Update, button_system.run_if(in_state(UIState::MainMenu)));
    }
}

#[derive(Component)]
struct MainMenuUIComponent;
...
fn despawn_ui(mut commands: Commands, mut ui: Query<Entity, With<MainMenuUIComponent>>) {
    commands
        .entity(ui.get_single_mut().expect("MainMenu UI Not Found!"))
        .despawn_recursive();
}
